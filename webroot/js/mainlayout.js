...
function setPageChange(section_id, pageNo, addFlag=null) {
    console.log($("[id^=right_set-"+section_id+"]"));
    if($("[id^=right_set-"+section_id+"]").length){
        ...
        $("[id^=section-"+sectionId[1]+"][name$=\\[id\\]]").each(function(){
            ...
            if (section[sectionId[3]].sd_section_structures[sectionStructureK].sd_field.sd_field_values.length>=1){
                ...
                var maxindex=0;
                console.log("kkk");
                console.log(section[sectionId[3]].sd_section_structures[sectionStructureK].sd_field.sd_field_values);
...
function paginationReady(){
    $("[id^=pagination-l2").each(function(){
        ...
        var max_set_no  = 0 ;
        $(child_section_id).each(function(k, v){
            console.log($("[id^=add_set-"+v+"]"));
            console.log(v);
            if($("[id^=add_set-"+v+"]").length){
                max_set_no = Math.max($("[id^=add_set-"+v+"]").attr('id').split('-')[5], max_set_no);
            }
            $("[id^=pagination-section-"+v+"]").hide();
        });console.log("max_set_no"+max_set_no);
        if (max_set_no==0) {
    ...
    $("div[id^=section-"+sectionId+"-field]").each(function(){
        ...
        if($(this).find("[name$=\\[field_value\\]]").length){
            ...
            if((field_type!="radio")&&(field_type!="checkbox")){
                 ...
                 console.log("not checked"+field_value);
            }else{$(this).find("[name$=\\[field_value\\]]").each(function(){
                if($(this).prop('checked')){ field_value = $(this).val();console.log("radio"+field_value);}
            })}
    ...
    $.ajax({
        ...
        success:function(response){
            ...
            for(var k in request){
                ...
                $("[id$=field-"+k+"]").children("[id^=section-"+sectionId+"-sd_section_structures]").val(savedArray[k]);
                var max_set_no  = 0
                ...
                request[k].id = savedArray[k];
                section[section_Id[2]].sd_section_structures[sectionStructureK].sd_field.sd_field_values[fieldvalueK] = request[k];
                console.log( request[k]);
            }
            // console.log("newPageFlag"+newPageFlag);
            if(newPageFlag){
                pageNew = 0;
                if($("[id^=add_set-"+sectionId+"]").length){
                    add_set_id = $("[id^=add_set-"+sectionId+"]").attr('id').split('-');
                    pageNew=Number($("[id^=add_set-"+sectionId+"]").attr('id').split('-')[5])+1;
                    ...
                    $(section).each(function(k,v){
                        if(v.id==sectionId) sectionKey = k;
                    });
                    var text= "";
                    text += "<div role=\"button\" id=\"add_set-"+sectionId+"-sectionKey-"+sectionKey+"-setNo-1\" onclick=\"setPageChange("+sectionId+",1,1)\" class=\"float-right px-3 mx-3 btn btn-outline-info\" title=\"Add new\">Add</div>";
                    text +=  "<nav class=\"d-inline-block float-right\" title=\"Pagination\" aria-label=\"Page navigation example\">";
                    text +=  "<ul class=\"pagination mb-0\">";
                    text +=     "<li class=\"page-item\" id=\"left_set-"+sectionId+"-sectionKey-"+sectionKey+"-setNo-1\" onclick=\"setPageChange("+sectionId+",0)\">";
                    text +=     "<a class=\"page-link\" aria-label=\"Previous\">";
                    text +=         "<span aria-hidden=\"true\">&laquo;</span>";
                    text +=         "<span class=\"sr-only\">Previous</span>";
                    text +=     "</a>";
                    text +=     "</li>";
                    text +=     "<li class=\"page-item\" id=\"section-"+sectionId+"-page_number-1\" onclick=\"setPageChange("+sectionId+",1)\"><a class=\"page-link\">1</a></li>";
                    text +=     "<li class=\"page-item\" id=\"right_set-"+sectionId+"-sectionKey-"+sectionKey+"-setNo-1\" onclick=\"setPageChange("+sectionId+",2)\">";
                    text +=     "<a class=\"page-link\" aria-label=\"Next\">";
                    text +=         "<span aria-hidden=\"true\">&raquo;</span>";
                    text +=         "<span class=\"sr-only\">Next</span>";
                    text +=     "</a>";
                    ...
                    text +=  "</ul>";
                    text +=  "</nav>";
                    $("#pagination-section-"+sectionId).html(text);
