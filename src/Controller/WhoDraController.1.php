    ...
    */
    public function search(){
        ...
        if($this->request->is('POST')){
            ...
            try{
                $qptb->select('test2.*, GROUP_CONCAT(DISTINCT atc.ATCText)')->from("test2");
                // $qptb->select($columns);
                // $qptb->from("mpdata");
                $qptb->join([
                    'atc' =>[
                        'table' =>'atc',
                        'type'=>'LEFT',
                        'conditions'=>['FIND_IN_SET(atc.ATCCode, REPLACE(test2.atc_code,\' \',\'\'))'],
                    ],
                    // 'dc' => [
                ...
                if( $atcCode != '' ){
                    $qptb->where(['test2.atc_code LIKE \''.$atcCode.'\'']);
                };
                if( $medProdID != '' ){
                    $qptb->where(['test2.medicinal_product_id LIKE \''.$MedProdID.'\'']);
                };
                if( $drugRecNo != '' ){
                    $qptb->where(['test2.DrugRecNo LIKE \''.$drugRecNo.'\'']);
                };
                if( $tradeName != ''){
                    $qptb->were(['test2.trade_name LIKE \''.$tradeName.'\'']);
                };
                if( $country != 'null' ){
                    $qptb->where(['test2.country LIKE \''.$country.'\'']);
                };
                
                if( $ingredient != '' ){
                    $qptb->where(['test2.ingredients LIKE \''.$ingredient.'\'']);
                };
                
                if( $formulation != '' ){
                    $qptb->where(['test2.formulation LIKE \''.$formulation.'\'']);
                };
                if( $strength != '' ){
                    $qptb->where(['test2.strength LIKE \''.$strength.'\'']);
                };
                $qptb->group('test2.DrugRecNo, test2.Seq1, test2.Seq2, test2.Seq3, test2.Seq4, test2.medicinal_product_id');
                $qptb->order(['test2.trade_name' => 'ASC']);
                $qptb->order(['test2.mah'=> 'ASC']);
