...
<div class="maintab container-fluid">
    ...
    <div class="dataentry">
        ...
        <?php
            ...
            foreach($sdSections->toarray() as $sdSectionKey => $sdSection_detail){
                $exsitSectionNo = displaySection($sdSection_detail, $exsitSectionNo, $sdSections);
            }
            ?>
    ...
    <?php
    function displaySection($section, $exsitSectionNo, $sdSections){
        if(empty($exsitSectionNo)) return null;
        if(in_array($section->id,$exsitSectionNo)){
            displaySingleSection($section);
            ...
            if(!empty($section->child_section)){ 
                ...
                foreach($child_array as $Key => $sdSectionKey){
                    $exsitSectionNo=displaySection($sdSections->toarray()[array_search($sdSectionKey,$exsitSectionNo)],$exsitSectionNo,$sdSections);
                }
            }
    ...
    function displaySingleSection($section){
        ...
        if($section->section_level == 2)
            echo "<div class=\"subtabtitle col-md-12\" id=\"$section->section_name\">".$section->section_name."</div>";
        elseif($section->section_level == 1 ){
            echo "<h3 class=\"mt-5\">".$section->section_name;
            echo"<button type=\"button\" id=\"save-btn".$section->id."\" class=\"float-right px-5 btn btn-outline-secondary\" title=\"Save this section\">Save</button>";
        ...
        foreach($section->sd_section_structures as $sd_section_structure =>$sd_section_structure_detail){
            ...
            ?>
                ...
                switch($sd_section_structure_detail->sd_field->sd_element_type->type_name){
                    case 'select':
                        echo "<select class=\"form-control\" id=id=\"select".$sd_section_structure_detail->sd_field->id."\" name=".$field_value_nameHolder.">";
                    ...
                    case 'checkbox':
                        ...
                        foreach($sd_section_structure_detail->sd_field->sd_field_value_look_ups as $option_no=>$option_detail){
                            ...
                            echo "<input class=\"checkboxstyle\" id=\"checkbox".$sd_section_structure_detail->sd_field->id."-option-".$option_detail['value']."\" name=".$field_value_nameHolder." value=";
                            echo $option_detail['value'];
                            if(!empty($sd_section_structure_detail->sd_section_values[$j])&&$sd_section_structure_detail->sd_section_values[$j]->field_value==$option_detail['value'])
                            echo " checked";                            
                    ...
                    case 'textarea':
                        ...
                        continue;  
                    case 'date':
                        echo "<input type=\"text\" class=\"form-control\" id=\"date".$sd_section_structure_detail->sd_field->id."\" value=";
                        echo (!empty($sd_section_structure_detail->sd_section_values[$j]))?$sd_section_structure_detail->sd_section_values[$j]->field_value:null;
                        echo ">";
                }
            echo"</div>";        
